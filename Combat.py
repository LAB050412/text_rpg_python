...
class Monster(Character):
    ...
    def __init__(self, name, grade,chapter):

        data = MONSTER_DATA[grade]

        super().__init__(
            name,
            int(data["hp"] + ((chapter-1)*25)),
            int(data["attack"] + ((chapter-1)*10)),
            data['lv'],
            int(data['exp'] + ((chapter-1)*25)),
            10,
            0
        )
...
def enter_map(player, map_data):
    ...
    for i, wave_data in enumerate(map_data["waves"]):
        ...
        monsters = [
            Monster(name, grade, map_data["chapter"])
